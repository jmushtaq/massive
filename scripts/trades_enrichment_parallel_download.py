...
Usage:
    ...
    python scripts/trades_enrichment_parallel_download.py --tickers_file data/universes/2022/combined_unique.csv --year 2022 --output data/combined --spawn 100 & 

    python scripts/trades_enrichment_parallel_download.py --tickers_file /tmp/processing_2025_tickers.csv  --year 2025 --output data/combined --spawn 100 --smart_resume --resume & 
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory to check for existing per-ticker files, e.g. 'data/quotes'. Script will skip tickers whose file already exists at <check>/<aggregate>/<year>/<ticker>_<year>_<folder>_trades.csv",
    )
...
def output_path(ticker: str, year: str, agg: str, parquet: bool = False, output_dir: str | None = None) -> Path:
    ...
    return base / "trades" / folder / year / f"{ticker}_{year}_{folder}_trades.{ext}"
...
def processing_path(ticker: str, year: str, agg: str, parquet: bool = False, output_dir: str | None = None) -> Path:
    ...
    ext = "parquet" if parquet else "csv"
    base = Path(output_dir) if output_dir else Path("data")
    return base / "trades" / folder / year / "processing" / f"{ticker}_{year}_{folder}_trades.{ext}"
...
def run_year(agg: str, year: str, args, overall_start: float) -> dict:
    ...
    state_base = Path(args.output) if args.output else Path("data") / "trades"
    ...
    log_fh = None
    if args.logs:
        log_base = Path(args.output) if args.output else Path("data") / "trades"
    ...
    if args.check:
        ...
        for t in all_tickers:
            check_file = Path(args.check) / folder_label / year / f"{t}_{year}_{folder_label}_trades.{ext}"
    ...
    report_base = Path(args.output) if args.output else Path("data") / "trades"
