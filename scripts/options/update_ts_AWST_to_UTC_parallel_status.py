"""
Display live status of a running or completed parallel AWST→UTC timestamp update.

Reads the state file written by update_ts_AWST_to_UTC_parallel.py.

Usage:
    python scripts/options/update_ts_AWST_to_UTC_parallel_status.py --year 2025 --aggregate 1sec
    python scripts/options/update_ts_AWST_to_UTC_parallel_status.py --year 2003-2025 --aggregate 1sec
    ...
    python scripts/options/update_ts_AWST_to_UTC_parallel_status.py --year 2025 --aggregate 1sec --kill
        --kill: kill all running workers and the dispatcher process
"""
...
def parse_years(year_arg: str | None) -> list[str] | None:
    if not year_arg:
        return None
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Show status of parallel AWST->UTC timestamp updates"
    )
    parser.add_argument(
        "--year",
        type=str,
        default=None,
        help="Year or year range filter (e.g. 2025 or 2003-2025; default: show all available state files)",
    )
    parser.add_argument(
        "--aggregate",
...
def show_status(state_path: Path):
    ...
    completed_ignored = sum(1 for v in completed.values() if v.get("status") == "ignored")
    ...
    print("    Updated:    %d" % completed_ok)
    print("    Ignored:    %d" % completed_ignored)
    ...
    if stats and "avg_time_s" in stats:
        print()
        print("  Duration stats:")
        print("    Fastest:    %.1fs" % stats["min_time_s"])
        print("    Slowest:    %.1fs" % stats["max_time_s"])
        print("    Average:    %.1fs" % stats["avg_time_s"])
...
def main():
    ...
    year_pats = parse_years(args.year) if args.year else ["*"]
    agg_pat = AGGREGATE_MAP[args.aggregate] if args.aggregate else "*"

    state_files = []
    for y in year_pats:
        pattern = f".parallel_state_{y}_{agg_pat}_utc"
