"""
Status monitor for compute_stocks_greeks_parallel_download.py.

Usage:
    python scripts/options/compute_stocks_greeks_parallel_status.py --year 2025
    python scripts/options/compute_stocks_greeks_parallel_status.py --year 2025 --watch
    python scripts/options/compute_stocks_greeks_parallel_status.py --year 2025 --kill
"""
...
def find_state_files(year, agg, odir=None):
    ...
    files = []
    for d in dirs:
        for p in d.glob(f".parallel_state_{year}_{folder}_options_greeks.json"):
            files.append(p)
    if not files:
        for d in dirs:
