"""
Compute implied volatility and Greeks for option chain CSV files.

Reads per-contract 1-min aggregate bars from chains CSV and computes
Black-Scholes implied volatility, delta, gamma, theta, vega, and rho.
Uses US Treasury yield curve for risk-free rate interpolation.

Writes separate output files — does not modify the input.

Output layout (default):
    data/options/chains/<agg>/<year>/<ticker>_<year>_<agg>_chains_greeks.csv

Usage:
    python scripts/options/compute_chain_greeks.py --tickers AAPL --year 2025
    python scripts/options/compute_chain_greeks.py --tickers AAPL --year 2025 --aggregate 1sec --resume
    python scripts/options/compute_chain_greeks.py --tickers_file data/universes/2025/combined_unique.csv --year 2025
"""
...
def bs_price(call_put: str, S: float, K: float, T: float, r: float, sigma: float) -> float | None:
...
def get_risk_free_rate(yields: list[dict], row_date: datetime.date, dte: int) -> float:
    ...
    maturities = sorted(k for k in best_entry if isinstance(k, int))
    if not maturities:
        return 0.04

    if dte <= maturities[0]:
        return best_entry[maturities[0]] / 100.0
    if dte >= maturities[-1]:
        return best_entry[maturities[-1]] / 100.0

    for i in range(len(maturities) - 1):
        if maturities[i] <= dte <= maturities[i + 1]:
            frac = (dte - maturities[i]) / (maturities[i + 1] - maturities[i])
            y1 = best_entry[maturities[i]]
            y2 = best_entry[maturities[i + 1]]
            if y1 <= 0 or y2 <= 0:
...
def parse_args():
    parser = argparse.ArgumentParser(description="Compute IV and Greeks for options chains")
    ...
    parser.add_argument("--year", type=str, required=True,
                        help="Year or year range (e.g. 2025)")
...
def output_path(ticker: str, year: str, agg: str, output_dir: str | None = None,
    ...
    if subdir:
        p = p / subdir
    return p / f"{ticker}_{year}_{folder}_chains_greeks.csv"


def input_path(ticker: str, year: str, agg: str, output_dir: str | None = None) -> Path:
    folder = AGGREGATE_MAP[agg][2]
    base = Path(output_dir) if output_dir else Path("data")
    return base / "options" / "chains" / folder / year / f"{ticker}_{year}_{folder}_chains.csv"
...
def compute_greeks(ticker: str, year: str, agg: str, output_dir: str | None,
    ...
    with open(in_path) as fin, open(out_path, "w", newline="") as fout:
        ...
        writer = csv.DictWriter(fout, fieldnames=GREEKS_HEADERS)
        writer.writeheader()
        ...
        for row in reader:
            ...
            try:
                ...
                call_put = row.get("call_put", "C").strip().upper()
            ...
            if close <= 0 or underlying <= 0 or strike <= 0 or dte <= 0:
                skipped_no_data += 1
                continue
            ...
            iv = bs_implied_vol(call_put, close, underlying, strike, dte / 365.0, r)
            if iv is None:
                skipped_no_data += 1
                continue
            ...
            writer.writerow({
                ...
                "implied_volatility": str(round(iv, 6)),
                "delta": str(g["delta"]),
                "gamma": str(g["gamma"]),
                "theta": str(g["theta"]),
                "vega": str(g["vega"]),
    ...
    logger.info("  [%s] %d rows parsed, %d with greeks, %d skipped (no data), %d skipped (ITM)",
                ticker, parsed, written, skipped_no_data, skipped_itm)
    return written
...
def main():
    ...
    logger.info("Chain Greeks compute: %d tickers, years=%s, aggregate=%s",
    ...
    for year in years:
        for i, ticker in enumerate(tickers, 1):
            ...
            if count == 0:
                logger.info("[%d/%d] %s (%s) -> no greeks computed (%.1fs)",
                ...
                all_downloaded += 1
                proc_path = output_path(ticker, year, agg, out_dir, subdir="processing")
                logger.info("[%d/%d] %s (%s) -> %d greeks (%.1fs)",
