...
def parse_args():
    p = argparse.ArgumentParser(description="Monitor chain greeks progress")
...
def find_state_files(year, agg, odir=None):
    ...
    dirs = [Path(odir) if odir else Path("data") / "options" / "chains"]
    files = []
    for d in dirs:
        for p in d.glob(f".parallel_state_{year}_{folder}_chains_greeks.json"):
            files.append(p)
    if not files:
        for d in dirs:
