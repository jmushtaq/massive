"""
Parallel dispatcher for compute_chain_greeks.py (in-place update).

Usage:
    python scripts/options/compute_chain_greeks_parallel_download.py --ohlcv_tickers --year 2025 --spawn 16
    python scripts/options/compute_chain_greeks_parallel_download.py --tickers AAPL --year 2025 --spawn 4 --exclude_tickers /tmp/skip.txt
    python scripts/options/compute_chain_greeks_parallel_download.py --ohlcv_tickers --year 2025 --spawn 16 --aggregate 1sec --exclude_tickers data/universes/excluded_tickers.txt
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="Parallel compute chain Greeks")
...
def is_final(ticker, year, agg, odir=None, inplace=False):
    """Check if implied_volatility column is populated."""
...
def main():
    ...
    sp = sb / "options" / "chains" / f".parallel_state_{year}_{folder}_chains_greeks.json"
    state = load_state(sp)
    ...
    if args.check:
        chk = [t for t in all_t
               if not (Path(args.check) / "options" / "chains" / folder / year
                       / f"{t}_{year}_{folder}_chains.csv").exists()]
        all_t = chk
    ...
    log("=" * 60)
    log(f"PARALLEL CHAIN GREEKS (in-place)  [year {year}]")
    ...
    def spawn(ticker):
        ...
        try:
            sf = open(f"/tmp/worker_chain_greeks_{ticker}_{year}.log", "w")
        ...
        while pending or active:
            ...
            for e in active:
                ...
                if ret is not None:
                    ...
                    save_state(sp, state)
                    # Finalize processing/ → destination
                    if ws == "ok":
